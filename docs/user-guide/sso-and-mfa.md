...
以下能力刻意不在本期范围内，请勿据此设计流程：P2P NAT 穿透、任意远程命令执行。ICMP echo 与内嵌 WireGuard VPN 网关属于独立特性（[ADR 0002](../architecture/adr/0002-public-ingress-and-embedded-vpn.md)，仅 `-tags vpn` 构建提供），同样不在本期范围内，也不与身份认证交互。
