...
补记说明：用户报告“通过代理请求的托管路由，大一点的 js 会加载中断”（`https://tm-6000d.claw.qihoo.net/assets/mode-DouGTm1Q.js`）。按 `AGENTS.md` 的紧急修复条款先止损再补记；红灯证据为实现过程中的真实观察，绿灯与门禁结果在补记时重新执行，见第 7 节。
