...
func TestStageEventRedactsSecretsTargetsAndPayloads(t *testing.T) {
	...
	if !strings.Contains(text, "[redacted]") || strings.Contains(text, "cn-east-1") {
		t.Fatalf("event did not redact metadata values: %s", text)
	}
	if !strings.Contains(text, `trace_id`) || strings.Contains(text, `TraceID`) {
		t.Fatalf("event JSON keys are not snake_case: %s", text)
	}
}
