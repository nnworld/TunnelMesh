...
func (a *API) handleTokens(w http.ResponseWriter, r *http.Request, principal auth.Principal, parts []string) {
	...
	case "reveal":
		if !isAdmin(principal) {
			a.denyToken(r, principal, record, "token_secret_reveal_admin_required")
			writeAPIError(w, http.StatusForbidden, "admin role required")
			return
		}
		if strings.TrimSpace(r.Header.Get("X-Token-Reveal-Confirm")) == "" {
			writeAPIError(w, http.StatusBadRequest, "X-Token-Reveal-Confirm is required")
			return
		}
		...
		if err := decodeJSON(r, &confirmation); err != nil || !confirmation.AcknowledgeRisk {
			writeAPIError(w, http.StatusBadRequest, "acknowledgeRisk=true is required")
			return
		}
		revealed, err := a.tokenService.Reveal(r.Context(), principal.UserID, record.ID)
		if err != nil {
			writeTokenError(w, err)
			return
		}
...
func writeTokenError(w http.ResponseWriter, err error) {
	...
	case errors.Is(err, errTokenSecretNotRecoverable):
		writeAPIError(w, http.StatusConflict, err.Error())
	case errors.Is(err, errTokenSecretUnavailable):
