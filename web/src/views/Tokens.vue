      ...
      <el-table :data="items" v-loading="loading" row-key="id">
        ...
        <el-table-column label="Actions" width="220" fixed="right">
          <template #default="{ row }">
            <el-button v-if="auth.isAdmin" link type="warning" :disabled="row.status === 'revoked'" @click="reveal(row)">Reveal</el-button>
...
import { createToken, listTokens, revokeToken, rotateToken, revealToken, type ServiceToken, type TokenType } from '../api/client'
...
async function rotate(row: ServiceToken) { try { await ElMessageBox.confirm('Rotating revokes the current token immediately. Continue?', 'Rotate token', { type: 'warning' }); const rotated = await rotateToken(row.id, crypto.randomUUID()); secret.value = rotated.secret || ''; await reload() } catch (error) { if (error === 'cancel' || error === 'close') return; ElMessage.error(error instanceof Error ? `Rotate failed: ${error.message}` : 'Rotate failed') } }
async function reveal(row: ServiceToken) { try { const confirmation = await ElMessageBox.prompt('Type REVEAL to confirm this audited secret read.', 'Reveal token secret', { inputPattern: /^REVEAL$/, inputErrorMessage: 'Enter REVEAL exactly', type: 'warning' }); const revealed = await revealToken(row.id, confirmation.value, crypto.randomUUID()); secret.value = revealed.secret; } catch (error) { if (error === 'cancel' || error === 'close') return; ElMessage.error(error instanceof Error ? `Reveal failed: ${error.message}` : 'Reveal failed') } }
async function revoke(row: ServiceToken) { try { await ElMessageBox.confirm('Revoked tokens cannot be used again. Continue?', 'Revoke token', { type: 'warning' }); await revokeToken(row.id); ElMessage.success('Token revoked'); await reload() } catch (error) { if (error === 'cancel' || error === 'close') return; ElMessage.error(error instanceof Error ? `Revoke failed: ${error.message}` : 'Revoke failed') } }
