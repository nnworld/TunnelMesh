-- v15 -> v16: embedded VPN gateway persistence (peer records and IP leases).
-- This step is numbered v16 rather than v15 because v0014_to_v0015 was already
-- published (v1.2.4) for the connection_epoch widening, and a published
-- incremental script must never be renumbered or rewritten.
-- Every statement is additive, so nothing written by a v15 binary becomes
-- unreadable and the migration can be applied while v15 is still serving.
...
-- SchemaVersion, so a v15 process restarted against a v16 database fails fast.
...
CREATE INDEX IF NOT EXISTS idx_vpn_peers_owner ON vpn_peers(owner_id, id);
CREATE INDEX IF NOT EXISTS idx_vpn_peers_node ON vpn_peers(node_id, status);
CREATE INDEX IF NOT EXISTS idx_vpn_peers_expires ON vpn_peers(expires_at);
...
CREATE INDEX IF NOT EXISTS idx_vpn_ip_leases_holder ON vpn_ip_leases(lease_holder, lease_expires_at);
CREATE INDEX IF NOT EXISTS idx_vpn_ip_leases_expires ON vpn_ip_leases(lease_expires_at);
