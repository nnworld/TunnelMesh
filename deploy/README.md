...
| `grafana/dashboard_schema_test.go`、`install/install_templates_test.go` | 产物一致性测试，随 `go test ./deploy/...` 执行 | [测试与验证](../docs/development/testing.md) |
...
分发附带许可文本与 NOTICE，缺失时 `build-release.sh` 直接失败）。`prometheus/`、`grafana/` 与
`certs/` 不进归档：前两者由运维自行挂载，后者是本地生成物且包含私钥。调整目录名或文件名前必须
