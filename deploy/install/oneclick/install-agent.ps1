# TunnelMesh Agent 一键安装（Windows）。设计与参数见 docs/deployment/oneclick-install.md。
# 本文件刻意保持薄：下载、校验、交互、渲染、服务注册全部在共享模块里。
# Windows 服务是机器级的，必须以管理员身份运行 PowerShell；否则退出码 3。
<#
.SYNOPSIS
  TunnelMesh Agent 一键安装（Windows 服务）。
.DESCRIPTION
  解析版本 → 下载并校验 Release 归档 → 安装 tunnelmesh-agent.exe → 渲染 agent.yaml 与服务 XML
  → check-config → 注册并启动 WinSW 服务 → 输出摘要。
  必须以管理员身份运行 PowerShell。
  退出码：0 成功、2 参数、3 preflight、4 下载、5 校验、6 配置校验、7 服务、8 卸载。
  agent token 不接受命令行参数（会进 PowerShell 历史与进程列表），只走 -TokenFile、
  -SecretEnvFile 或 TUNNELMESH_AGENT_TOKEN 环境变量。
.PARAMETER Version
  指定 vX.Y.Z；默认解析最新 Release。
.PARAMETER Yes
  非交互模式：所有提示取默认值；缺少必填项时以退出码 3 失败。
.PARAMETER ServerUrl
  Server 的 WebSocket 地址，必须以 /ws/agent 结尾。
.PARAMETER TokenFile
  从文件首行读取 agent service token；文件权限应只允许服务账户读取。
.PARAMETER Uninstall
  卸载服务与二进制，保留 C:\ProgramData\TunnelMesh 下的配置与日志。
.EXAMPLE
  .\install-agent.ps1
.EXAMPLE
  .\install-agent.ps1 -Yes -ServerUrl wss://tunnel.example.com/ws/agent -AgentId agent-01 -TokenFile C:\secure\agent.token
.LINK
