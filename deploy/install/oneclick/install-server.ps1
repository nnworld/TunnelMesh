# TunnelMesh Server 一键安装（Windows）。设计与参数见 docs/deployment/oneclick-install.md。
# 本文件刻意保持薄：下载、校验、交互、渲染、服务注册全部在共享模块里。
# Windows 服务是机器级的，必须以管理员身份运行 PowerShell；否则退出码 3。
<#
.SYNOPSIS
  TunnelMesh Server 一键安装（Windows 服务）。
.DESCRIPTION
  解析版本 → 下载并校验 Release 归档 → 安装 tunnelmesh-server.exe → 渲染 server.yaml 与服务 XML
  → check-config → 注册并启动 WinSW 服务 → 可选创建首个管理员 → 输出摘要。
  必须以管理员身份运行 PowerShell。
  退出码：0 成功、2 参数、3 preflight、4 下载、5 校验、6 配置校验、7 服务、8 卸载。
  敏感值（MySQL DSN、relay token、身份主密钥）不接受命令行参数，只走 -SecretEnvFile 或 TUNNELMESH_* 环境变量。
.PARAMETER Version
...
.EXAMPLE
  .\install-server.ps1
.EXAMPLE
  .\install-server.ps1 -Yes -SecretEnvFile C:\secure\tunnelmesh.env
.LINK
...
Invoke-TmMain -Role server -Bound $PSBoundParameters
